...
import pandas as pd
st.image("logo.jpg")# Insertar la imagen con el ancho calculado en píxeles
columna1, columna2, columna3  = st.columns(3) #separamos en 3 columnas
with columna1:
    '''### Endpoint 1'''
...
with columna2:
    if st.checkbox('EndPoint1', value=False):
        ...
        result_df = pd.read_csv('PlayTimeGenre.csv') #Abrimos Archivo
        valores_unicos = result_df['genres'].unique() # Obtener los valores únicos de la columna 'genres'
        key_genero = "selectbox_genero"# Mostrar la caja desplegable en la interfaz de Streamlit
        ...
        response_data = "Año de lanzamiento con más horas jugadas para {}: {}".format(genero, max_hours_year)# Construye el response_data
with columna3:
    try:
...
with columna2:
    if st.checkbox('EndPoint2', value=False):
        ...
        valores_unicos2 = consulta2['genres'].unique() # Filtrar el DataFrame por el género dado
        genero2 = st.selectbox('Selecciona un género:', valores_unicos2,)
        genre_data2 = consulta2[consulta2['genres'] == genero2]
        top_user = genre_data2.loc[genre_data2['hours_game'].idxmax()]['user_id']        # Encontrar al usuario con más horas jugadas para ese género
        ...
        mensaje_usuario = "Usuario con más horas jugadas para Género {}: {}".format(genero2, top_user) # Crear el mensaje con el resultado
...
with columna3:
    try:
        ...
        st.write("Aguardando Seleccion.")
#Autor Roman Brandariz https://www.linkedin.com/in/romanbrandariz/
st.write("¡Hola! Puedes encontrar mi perfil de LinkedIn [aquí](https://www.linkedin.com/in/romanbrandariz/).")